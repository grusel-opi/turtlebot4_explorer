...
public:
    Explorer() : Node("Turtlebot4_explorer")
    {
        ...
        waypoint_navigator_ = rclcpp_action::create_client<WaypointAction>(this, "/follow_waypoints");
        ...
        get_global_costmap_client_ = create_client<nav2_msgs::srv::GetCostmap>("/global_costmap/get_costmap", rmw_qos_profile_services_default, callback_group_);

        coverage_step_size_w_ = 0.5;
        current_coverage_pose_nr_ = 0;

        RCLCPP_INFO(get_logger(), "Creating wall timer for start routine..");
        start_timer_ = this->create_wall_timer(2s, [this]() {
            start();
        });
...
private:
    ...
    std::shared_future<rclcpp_action::ClientGoalHandle<NavAction>::SharedPtr> nav_action_future_goal_handle_;
    std::shared_future<rclcpp_action::ClientGoalHandle<WaypointAction>::SharedPtr> waypoint_future_goal_handle_;
    ...
    int current_coverage_pose_nr_;
    double coverage_step_size_w_; // min step size in world scale to execute coverage pattern
    ...
    rclcpp::Client<nav2_msgs::srv::GetCostmap>::SharedPtr get_global_costmap_client_;
    ...
    void explore() {
        ...
        nav_action_future_goal_handle_ = pose_navigator_->async_send_goal(goal, send_goal_options);
    ...
    void findFrontiers() {
        ...
        RCLCPP_INFO(get_logger(), "[findFrontiers] current position %f, %f", position.x, position.y);
    ...
    void navigationResponseCallback(
            const rclcpp_action::ClientGoalHandle<NavAction>::SharedPtr &goal_handle) {
        if (goal_handle) {
    ...
    void navigationResultCallback(const rclcpp_action::ClientGoalHandle<NavAction>::WrappedResult &result) {
    ...
    void poseCallback(geometry_msgs::msg::PoseWithCovarianceStamped::UniquePtr poseMsg) {
    ...
    void calculateCoverage() {
        ...
        auto status = result.wait_for(5s); // 5s should be very generous
        ...
        // }
        // marker_array_publisher_->publish(marker_array_);
        // RCLCPP_INFO(get_logger(), "published poses number: %ld", marker_array_.markers.size());
        ...
        // marker_array_publisher_->publish(marker_array_);
        // RCLCPP_INFO(get_logger(), "published positions number: %ld", marker_array_.markers.size());


        // Attempt to step from current position into free space to ensure (coutner-) clockwise execution pattern (in case of cameras mounted to only one side) 
        ...
        // if (!costmap.worldToMap(first_cov_pos.x, first_cov_pos.y, mx, my)) {
        //     RCLCPP_ERROR(get_logger(), "[calculateCoverage] worldToMap failed, should not happen!");
        //     return;
        // }
        // first_pos_cost = costmap.getCost(mx, my);

        // if (!costmap.worldToMap(query.x, query.y, mx, my)) {
        //     RCLCPP_ERROR(get_logger(), "[calculateCoverage] worldToMap failed, should not happen!");
        //     return;
        // }
        ...
        //     current_coverage_pose_nr_ = 0;
        // } else {
        //     current_coverage_pose_nr_ = coverage_positions_sorted_.size() - 1;
        // }

        // RCLCPP_INFO(get_logger(), "first_pos_cost: %d, query_pos_cost: %d, current_coverage_pose_nr_: %d, grad: %d", first_pos_cost, query_pos_cost, current_coverage_pose_nr_, grad);
        
        executeCoverageViaWaypoints();
        // executeCoverage();
    }
    ...
    void executeCoverageViaWaypoints() {
        ...
        for (const auto & p : coverage_positions_sorted_) {
            for (const auto & o : orientations_deg) {
                ...
                waypoint_pose.pose.orientation.z = std::sin((o / 360. * (2*M_PI)) / 2);
                waypoint_pose.pose.orientation.w = std::cos((o / 360. * (2*M_PI)) / 2);
        ...
        send_goal_options.goal_response_callback = [this](const auto& goal_handle) {
            if (goal_handle) {
                RCLCPP_INFO(get_logger(), "[RESPONSE] Waypoint goal accepted by server.");
            } else {
                RCLCPP_ERROR(get_logger(), "[RESPONSE] Waypoint goal was rejected by server.");
            }
        };
        ...
        waypoint_future_goal_handle_ = waypoint_navigator_->async_send_goal(goal, send_goal_options);
    }
    ...
    void executeCoverage() {
        ...
        send_goal_options.goal_response_callback = [this](const auto& goal_handle) {
            if (goal_handle) {
                ...
                if (current_coverage_pose_nr_ < (int) coverage_positions_sorted_.size() && current_coverage_pose_nr_ >= 0) {
                    executeCoverage();
                } else {
                    RCLCPP_INFO(get_logger(), "[executeCoverage] Seems like coverage is completed! current_coverage_pose_nr_: %d", current_coverage_pose_nr_);
                }
            }
        };
    ...
    void cheapTSP(std::vector<geometry_msgs::msg::Point>& positions) {
        ...
        coverage_positions_sorted_.push_back(current_pose_->pose.pose.position);
    }
};
